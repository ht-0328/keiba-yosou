...
def dataset_builder(con: duckdb.DuckDBPyConnection) -> DatasetBuilder:
    ...
    この予想の決めごとは3つ: 出走した全頭を入れる（``RunnerSelector``）、3着以内なら 1（共通の
    ``Top3TargetBuilder``）、特徴量は 71個（``CATALOG``）。ほかの予想は、同じ ``DatasetBuilder`` に別のクラスを渡す（設計書 04）。
    """
    return DatasetBuilder(
        HistoryRecordsLoader(con), RaceRecordsLoader(con),
        RunnerSelector(), Top3TargetBuilder(), FeatureBuilder(CATALOG),
    )
