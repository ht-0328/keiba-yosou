...
| 場所 | 中身 |
|---|---|
| ``feature_catalog.py`` | どの予想でも使う 71個の一覧（``BASE_FEATURES``）、人気を使う予想が足す4個（``POPULARITY_FEATURES``）、予想ごとの一覧（``FeatureCatalog``） |
| ``Feature``・``FeatureKind`` | 一覧の1行と、その型 |
| ``FeatureGroup`` | まとまりのクラスに共通の決まり（インターフェース） |
...
| ``group/`` | まとまり A〜J ごとに1クラス（J は人気を使う予想だけが渡す） |
| ``history/`` | 過去の記録から数える部品（開催日より前のものだけを使う決まりを、ここで守る） |
| ``as_numbers`` | 数の列を小数の列にする関数（欠損値のそろえ方を1か所にする） |
...
from .feature_catalog import BASE_FEATURES, POPULARITY_FEATURES, FeatureCatalog
...
__all__ = [
    ...
    "RecentRunSummary", "BASE_FEATURES", "POPULARITY_FEATURES", "FeatureCatalog", "Feature", "FeatureKind",
    "FeatureGroup",
    "WORKOUT_WINDOW_DAYS", "PEOPLE_WINDOW_DAYS", "as_numbers",
]
