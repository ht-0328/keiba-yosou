...
二値分類（手本・人気馬・穴馬の予想）は、確率の当たり具合を測る一般的な指標（ログ損失・AUC・Brier スコア）と、
各レースで確率がいちばん高い馬の、目的変数が 1 だった割合を出す。
多クラス分類（荒れ具合の予想）は、正解率・マクロ F1・クラスのずれの平均・ログ損失・累積確率の AUC・混同行列を出す
（荒れ具合の設計書 16）。
