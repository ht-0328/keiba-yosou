...
class PredictCommand:
    ...
    def run(self, args: argparse.Namespace) -> list[Table]:
        with db.open_db(args.db) as con:
            ...
            workflow = PredictionWorkflow(
                ...
                PopularityApplier(odds_repository), OddsResolver(odds_repository), self._judges(args),
            )
            prediction = workflow.run(race_id, args.timing, self._given_popularity(args), self._given_odds(args))
        extra = [POPULARITY_RANK, FAVORITE_BAND, BASE_OUT, DANGER, IS_DANGER]
    ...
    def _judges(self, args: argparse.Namespace) -> dict[PredictionTiming, DangerJudge]:
        """学習のときに保存した、時点ごとの危険の判定の線。無ければ（前の版で学習したモデル）危険と判定しない。"""
        return {timing: DangerJudge(lines) for timing, lines in DangerThresholdRepository(args.models).load().items()}
