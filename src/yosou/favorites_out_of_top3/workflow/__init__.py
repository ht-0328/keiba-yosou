...
| ``TIMINGS`` | この予想が学習し、予測を出す時点（前日・当日）。``TIMING_CHOICES`` はその書き方の案内 |
| ``SEGMENTS`` | 学習データの分け方（1番人気・2〜3番人気・4〜5番人気を別のモデルで学ぶ） |

学習の流れ（``TrainingWorkflow``）は、どの予想でも同じなので ``yosou.shared.workflow``。人気の扱いが
予想ごとに違う予測の流れだけを、ここに置く。学習の結果の入れ物（``TrainingReport``）は ``yosou.shared.evaluation``。
"""

from .model_segments import SEGMENTS
from .prediction_timings import TIMING_CHOICES, TIMINGS
from .prediction_workflow import PROBABILITY, PredictionWorkflow

__all__ = ["PredictionWorkflow", "PROBABILITY", "TIMINGS", "TIMING_CHOICES", "SEGMENTS"]
