...
class MaterialsLoader:
    """``predict_all.py`` が書いた4つの CSV（``<予測フォルダ>/<モデル名>.csv``）と、事実表のレースの属性、複勝の確定オッズから ``RaceMaterials`` を作る。
    ...
    def load(self, days: RaceDayRange) -> RaceMaterials:
        ...
        place_odds = FinalOddsRepository(self._con, TicketType.PLACE).read(days)
        runners = RunnerTableBuilder().build(form, favorites, longshots, place_odds)
