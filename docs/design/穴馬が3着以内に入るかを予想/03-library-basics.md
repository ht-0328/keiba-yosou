...
`predict_proba()` の2列目は、手本と同じ「3着以内に入る確率」である（[10-target.md](10-target.md#作り方)）。危険な人気馬の予想では「4着以下になる確率」だったが、この予想では手本と同じ向きに戻る。どちらも目的変数が 1 になる確率で、モデルの使い方は変わらない。

## この予想で使う機能と、書いてある文書

| この予想でやること | 使う機能 | この予想での使い方 |
|---|---|---|
| 過去のレースの穴馬で学習する | `fit()` | [05-sequence.md](05-sequence.md#図1-学習)、[12-lightgbm.md](12-lightgbm.md)・[13-catboost.md](13-catboost.md) |
...
| 今日のレースの穴馬の確率を出す | `predict_proba()` の2列目 | [05-sequence.md](05-sequence.md#図2-予測) |
