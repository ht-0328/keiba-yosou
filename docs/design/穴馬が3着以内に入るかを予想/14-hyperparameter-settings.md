...
**結論: 設定ファイル（TOML 形式）の形も、指定のしかたの決まりも、手本とまったく同じである。この予想で違うのは、初期値のファイルの置き場所だけである。**
...
設定ファイルの中身は、初期値も項目の名前も手本と同じである。3つの予想で違う値を使いたくなったときは、それぞれの初期値のファイルを書き換える。設定ファイルを読む部品（ファイルを読む・名前を確かめる・初期値に重ねる）は共通なので、書き方の決まりは1か所で守られる。

`--zone`（出力を絞る区分）は、予測のコマンドの引数であって、設定ファイルには書かない。学習には関係しないためである（[07-prediction-timing.md](07-prediction-timing.md#予測のときの人気の与え方)）。
