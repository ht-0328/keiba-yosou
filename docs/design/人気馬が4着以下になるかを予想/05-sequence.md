# 05 学習と予測のやりとり（シーケンス図）

**この文書で示すこと:** 学習と予測のとき、利用者・jvdata-store・元DB と、[04-classes.md](04-classes.md) のクラスが、どの順に、どのメソッドを呼ぶか。

**結論: 流れは「学習」と「予測」の2つに分かれる。** 学習は `TrainingWorkflow` が進め、2つの時点（前日・当日）ごとに LightGBM と CatBoost を学習させて保存する。予測は `PredictionWorkflow` が進め、**利用者から受け取った人気を出走の行に当ててから**、人気馬の行だけの予測用データを作り、保存した2つのモデルの予測確率を平均する。

- 図に出てくるものと、図の読み方（凡例）は [手本の 05 の「図に出てくるもの」](../近走と適性から3着以内を予想/05-sequence.md#図に出てくるもの) と [「図の読み方」](../近走と適性から3着以内を予想/05-sequence.md#図の読み方) を参照。この予想で増えるのは、利用者が渡す人気と、`AnnouncedOddsRepository` だけである。
- public メソッドの中の判断（if 文による分かれ道）は [06-flowchart.md](06-flowchart.md) を参照。2種類の図の使い分けは [01-overview.md](01-overview.md#図の使い分け) を参照。
- **リポジトリとのやりとりは、手本とまったく同じである。** 学習データを集める流れは [手本の 05 の図3](../近走と適性から3着以内を予想/05-sequence.md#図3-記録を集めるリポジトリとのやりとり)、1レースの記録を集めて速報を反映する流れは [手本の 05 の図4](../近走と適性から3着以内を予想/05-sequence.md#図4-1レースの記録を集める速報の反映) を参照。この文書では描き直さない。

## 図1. 学習

```mermaid
sequenceDiagram
    actor U as 利用者
    participant W as TrainingWorkflow
    participant H as HyperparameterSettings
    participant D as DatasetBuilder
    participant L as HistoryRecordsLoader
    participant FS as FavoriteSelector
    participant F as FeatureBuilder
    participant T as OutOfTop3TargetBuilder
    participant S as PeriodSplitter
    participant MR as ModelRepository
    U->>W: run（設定ファイルのパス）
    W->>H: load（設定ファイルのパス）
    H-->>W: 設定
    W->>D: build_training_data（期間）
    D->>L: load（ウォームアップの始まり。既定は 2020年1月1日）
    L->>L: リポジトリを順に呼んで記録を集める（手本の 05 の図3）
    L-->>D: 出走の記録（レースの全出走馬）
    D->>FS: training_samples（出走の行、学習データの始まり）
    FS->>FS: 入れる行を選び、「人気馬か」を足す（06-flowchart.md の図1）
    FS-->>D: サンプルの候補（レースの全出走馬）
    D->>F: build（記録、当日）
    F-->>D: 特徴量 75個（レース内順位は、レースの全出走馬から計算）
    D->>FS: keep_samples（特徴量の付いた行）
    FS-->>D: 人気馬の行だけ
    D->>T: build（人気馬の行）
    T-->>D: 目的変数（4着以下なら 1）
    D-->>W: 学習データ（1行 = 1レースの人気馬1頭）
    W->>S: split（学習データ）
    S-->>W: 学習データ・検証データ・テストデータ
    loop 2つの時点（前日・当日）ごと
        W->>W: LightGbmModel を作り、その時点の列だけで学習させる（12-lightgbm.md の 4.）
        W->>W: CatBoostModel を作り、その時点の列だけで学習させる（13-catboost.md の 4.）
        W->>MR: save（時点、2つのモデル、設定）
        MR-->>W: 保存した
    end
    W->>W: 検証データで当たり具合を確かめる（次の設計書）
    W-->>U: 確かめた結果
```

**説明。** 利用者が設定ファイルのパスを付けて `TrainingWorkflow.run()` を呼ぶ。`TrainingWorkflow` は、設定を読み（[14-hyperparameter-settings.md](14-hyperparameter-settings.md)）、渡された期間で `DatasetBuilder` に学習データを作らせ、`PeriodSplitter` で時期に分ける。

手本と違うのは、**人気馬に絞るのが、特徴量を作ったあとになる**ところである。`FavoriteSelector.training_samples()` は、障害・取消・期間でふるったうえで「人気馬か」の列を足すだけで、行は減らさない。レース内順位（まとまり G）を、そのレースの全出走馬から計算するためである（[08-training-data.md](08-training-data.md#3-どのサンプルを入れるか)）。特徴量ができたあと、`keep_samples()` が人気馬の行だけを残し、`OutOfTop3TargetBuilder` が目的変数を付ける。

学習データは、当日の時点の特徴量 75個で作る。前日のモデルには、そのうち、その時点で使う列だけを渡す（[07-prediction-timing.md](07-prediction-timing.md#時点ごとに使う特徴量)）。2つの時点ごとに2つのモデルを学習させるので、モデルは合わせて4つになる。

## 図2. 予測

前日か当日に、1レースの人気馬を予測するときの流れ。

```mermaid
sequenceDiagram
    actor U as 利用者
    participant JS as jvdata-store
    participant DB as 元DB
    participant W as PredictionWorkflow
    participant D as DatasetBuilder
    participant L as RaceRecordsLoader
    participant FS as FavoriteSelector
    participant PA as PopularityApplier
    participant OR as AnnouncedOddsRepository
    participant F as FeatureBuilder
    participant C as RequiredInfoCheck
    participant MR as ModelRepository
    participant E as EnsembleModel
    U->>JS: jvstore sync（出馬表・出走別着度数・調教）
    JS->>DB: 書き込む
    U->>JS: jvstore realtime（開催日）
    JS->>DB: 馬場状態・出馬表の変更・締め切り前のオッズ（当日は馬体重も）を書き込む
    U->>W: run（レースID、時点、人気（--pops 馬番:人気））
    W->>PA: resolve（レースID、渡された人気）
    opt 利用者が人気を渡さなかったとき
        PA->>OR: read（レースID）
        OR->>DB: SQL（締め切り前の単勝オッズ）
        OR-->>PA: 馬番ごとのオッズ（無ければ、無し）
    end
    PA-->>W: 馬番 → 人気（決められなければ、無し。06-flowchart.md の図2）
    W->>D: build_prediction_data（レースID、時点、馬番 → 人気）
    D->>L: load（レースID、馬番 → 人気）
    L->>L: 速報を読み、出走馬の記録を集めて、速報を反映し、渡された人気を当てる（手本の 05 の図4）
    L-->>D: 出走の記録
    D->>FS: prediction_runners（出走の行、レースID）
    FS->>FS: 「人気馬か」を足す（人気が1頭も分からなければ止める）
    FS-->>D: 「人気馬か」を足した行
    D->>F: build（記録、時点）
    F-->>D: その時点で使う特徴量
    D->>FS: keep_samples（特徴量の付いた行）
    FS-->>D: 人気馬の行だけ
    D->>C: check（特徴量）
    C-->>D: 要る情報（馬番・馬場状態・馬体重）はそろっている
    D-->>W: 予測用データ（1行 = 人気馬1頭）
    W->>MR: load（時点）
    MR-->>W: その時点の LightGbmModel と CatBoostModel
    W->>E: predict_proba（予測用データ）
    E->>E: 2つのモデルの予測確率を出して平均する（12-lightgbm.md・13-catboost.md の 5.）
    E-->>W: 1頭ずつの予測確率
    W-->>U: 人気馬ごとの「4着以下になる確率」（高い順）
```

**説明。** 利用者は、まず jvdata-store の `jvstore sync` で出馬表・出走別着度数・調教を、`jvstore realtime --date <開催日>` で速報（馬場状態、締め切り前の単勝オッズ、当日は馬体重も）を取り込む。オッズは発表時刻ごとの断面で `o1` に残るので、予測の直前に取り直すほど、締め切り後の人気に近づく。次に、レースIDと時点と人気を付けて `PredictionWorkflow.run()` を呼ぶ。

手本と違うのは、**人気を決める段（`PopularityApplier`）が、予測用データを作る前に入る**ところである。利用者が `--pops` で渡した人気があればそれを使い、無ければ `AnnouncedOddsRepository` で元DB の締め切り前のオッズを読んで人気を作る。どちらも無ければ「無し」を返し、元DB の出走の行に入っている人気（終わったレースの確定単勝人気）がそのまま使われる。それも空なら、`FavoriteSelector` が、人気を渡すよう案内して止める（判断は [06-flowchart.md](06-flowchart.md#図2-予測用データに人気を当てる)）。

決めた「馬番 → 人気」を出走の行に当てるのは、共通の `RaceRecordsLoader` である（手本の 05 の図4 の `RaceEntryTableRepository`）。学習と同じ SQL で作った出走の行の、単勝人気の列を書き換えるので、学習と予測で列の意味がずれない。そのあとの流れは学習と同じで、「人気馬か」を足す → 特徴量を作る → 人気馬の行だけ残す、の順に進む。`RequiredInfoCheck` の仕事は手本と同じで、馬番・馬場状態・馬体重がそろっているかを確かめる。

予測用データも、学習と同じ `DatasetBuilder` と `FeatureBuilder` で作る（[11-leak-prevention.md](11-leak-prevention.md#決まり) の 4）。

## 今は無く、これから作るところ

| 図の中の部分 | 今の状態 |
|---|---|
| `shared` のクラス | 作った（`src/yosou/shared/`。手本の `src/yosou/form_aptitude_top3/` から移した。[04-classes.md](04-classes.md#1-共通の部品とこの予想だけの部品の分け方)） |
| この予想のクラス（`FavoriteSelector` など） | 作った（`src/yosou/favorites_out_of_top3/`） |
| `AnnouncedOddsRepository` が読むオッズ | 作った。断面は jvdata-store の `jvstore realtime --date <開催日>`（速報オッズ `0B31` → `o1`）で入る。取り忘れたレースは `--pops` で人気を渡す（[07-prediction-timing.md](07-prediction-timing.md#予測のときの人気の与え方)） |
| 学習データ・検証データ・テストデータの期間の分け方と、評価指標 | 手本と同じく、次の設計書で決める |
| アンサンブルの平均のしかた | 手本と同じく、次の設計書で決める |
| 予測確率から「危険」と判定する線引き | 決めない。確率をそのまま高い順に出す（[15-decisions.md](15-decisions.md#10-危険と判定する線引きを設計書に入れるか)） |

## 文書情報

| 項目 | 内容 |
|---|---|
| 作成日 | 2026-09-22 |
