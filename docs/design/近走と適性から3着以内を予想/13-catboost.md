...
| 決めること | 結論 |
|---|---|
| 1. 学習データの渡し方 | カテゴリ特徴量は文字列のまま `cat_features` に指定し、欠損値だけ文字列「不明」にする。出走の少ない値はまとめない |
...
| カテゴリ特徴量の欠損値 | 文字列「不明」にする | CatBoost は、カテゴリ特徴量に NaN があると学習できない。「なし」にしないのは、特徴量の値として意味を持つ「なし」（直近の調教のコースの「調教なし」）と混ざるため（[09-features.md](09-features.md) の I） |
...
**説明。** 欠損値だけを文字列「不明」に置き換え、それ以外はすべて文字列にする。LightGBM と違い、出走の少ない値や、学習のときに無かった値を「その他」にまとめる分かれ道は無い。
