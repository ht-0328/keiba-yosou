...
| カテゴリ特徴量の、出走が少ない値 | 学習データでの出走が 2000 回未満（設定ファイルの `min_category_count` で変えられる）の値を「その他」にまとめる。対象は騎手・調教師・父・父の父・母の父 | LightGBM は数回しか出てこない値でも分かれ道を作れてしまい、過学習しやすい。下の「2000 にした理由」を参照 |
...
- `fit(学習データ)`: 学習データでの出走が 2000 回以上の値を集めて、カテゴリの一覧を作る。分かれ道は、この「2000 回以上か」の1つだけである。
...
**説明。** 一覧に無い値は、学習データで出走が 2000 回に満たなかった値か、学習のあとに出てきた値（学習のあとにデビューした騎手など）である。どちらも「その他」にする。競馬場・芝ダのように値の種類が少ないカテゴリ特徴量は、この手順を通さず、そのまま `category` 型にする。

### 2000 にした理由

初めは 50 にしていたが、検証データでの当たり具合が下がっていた。学習データ（2024年1月〜2025年6月の 6.9 万行）では、
LightGBM が判断の半分を騎手・調教師・血統の**名前**に置いてしまい、数百走ぶんの偶然を覚え込んでいた（過学習）。

2000 にすると、この学習データ（2024年1月から）では騎手・調教師の名前が1つも残らず、血統も大きな系統だけが残る。
そのぶん LightGBM は近走・前走を見るようになり、検証データでの当たり具合が上がった（2つの期間で確かめた）。
名前そのものは、扱いの上手な CatBoost（順序付きターゲット統計）が引き続き使う。

**この値は学習データの長さで変わる。** 期間を延ばして1つの名前に数千走ぶんのデータが集まれば、
名前を覚えても偶然ではなくなるので、小さい値に戻して比べ直す。

**学習データの期間を延ばして比べ直した結果（2026-09-22）。** 学習データの始まり3つ（2024年1月・2021年8月・2017年1月。[08-training-data.md](08-training-data.md) の「既定を 2021年8月からにした理由」）と、検証データの期間2つの、6通りで 50 と 2000 を比べた。

- 6通りのすべてで、2000 のほうが当たり具合が良かった。**2000 のままにする。**
- 差は、学習データが長いほど小さくなる。それでも 10年ぶん（約 39 万行）で 50 が上回ることはなかった。
- 期間を延ばすと、2000 回以上出走する名前が増え、2000 のままでも名前が残るようになる。既定の 2021年8月からでは、騎手は 30人あまりの名前が残り、調教師は残らない。血統は父・父の父・母の父がそれぞれ 20ほど残る。LightGBM は、出走の多い名前だけを使うことになる。
