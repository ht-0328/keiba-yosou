...
| 4. 特徴量の重み | まとまり A〜K の重みを掛ける。今はすべて 1.0。重み 0 のまとまりは使わない |
...
| 特徴量の種類 | 例 | 距離に使う形 | 理由 |
|---|---|---|---|
| 数の特徴量 | 単勝オッズ、前走の人気、斤量、騎手の近1年の3着以内の割合 | 欠損値を埋め、標準化する（下の 2.・3.） | 大小に意味がある |
...
標準化したあとの列に、その特徴量のまとまり（A〜K）の重みを掛ける（方針の `[features.group_weights]`）。**今はすべて 1.0** である（[15-decisions.md の 5](15-decisions.md#5-特徴量の重み)）。重みを 0 にしたまとまりは、距離に使わない。

距離は列ごとの差の2乗の和なので、列が多いまとまりほど距離に強く効く。市場の評価（K）は3個だけで、近走（D・E）は 20個ある。K を重くすると、単勝オッズの近い1番人気どうしが似た馬になりやすい。
