...
k近傍法の距離は、列ごとの差を足し合わせる。斤量（50〜60 くらい）と馬体重（400〜550 くらい）をそのまま足すと、数の大きい馬体重ばかりが距離を決めてしまう。そこで、列ごとに平均 0・ばらつき 1 にそろえる（標準化）。
...
| | 変換前 | 変換後 |
|---|---|---|
| 斤量 | 55.0, 57.0, 56.0 | −1.22, 1.22, 0.00 |
...
# 架空の1番人気 300頭（1つの単位の学習データのつもり）。本番は、オッズを除いた 69個（当日）の特徴量から作る
n = 300
horses = pd.DataFrame({
    "斤量":       rng.choice([54.0, 55.0, 56.0, 57.0, 58.0], n),
...
target = pd.DataFrame({"斤量": [55.0], "前走の人気": [1.0], "前走の着順": [3.0]})
...
馬券内 の頭数 80
馬券外 の頭数 220
{'勝利': 84.3, '馬券内': 93.3, '馬券外': 58.0}
複勝だけ
```

- 例のために、特徴量を3つ（どれもオッズではない列）に絞り、着順を乱数で付けた（そのため勝利の頭数が本物の1番人気より少ない）。本番の1単位は数百〜数千頭で、特徴量は当日なら 69個使う。この予想はオッズなしで予想するので、単勝オッズの列は距離に使わない（[08-training-data.md](08-training-data.md)・[12-neighbor-distance.md](12-neighbor-distance.md)）。
- 3. の「対象の馬」はグループの外の馬なので、自分を除く必要は無い（`n_neighbors=k`）。
- 例では、この架空の馬は馬券内のグループにいちばん近い（93.3点）。馬券外（58.0点）は馬券内より低いので消さず、勝利（84.3点）も馬券内より低いので「複勝だけ」になった。
