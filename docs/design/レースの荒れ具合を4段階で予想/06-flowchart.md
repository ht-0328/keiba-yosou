...
**説明。** 手本の `RequiredInfoCheck` は「馬番・馬場状態・馬体重・オッズ」の4つを確かめ、オッズは渡された馬だけで計算する。この予想では、まとまり B が全頭のオッズから作る値（10倍未満の頭数、市場勝率の合計）なので、1頭でも欠けると値の意味が変わる。そのため、3つ目の問いで全頭ぶんを確かめる。一部の馬に無ければ `FieldOddsCheck` が予測用データを作る前に止め、全頭に無ければ `RequiredInfoCheck` が取り込み方を案内する（案内文は、作られるときに受け取る。[04-classes.md](04-classes.md#2-共通の部品に足すもの変えるもの)）。馬体重は、D の「1番人気の馬体重の増減」にだけ使うので、1番人気の分だけを確かめる。
